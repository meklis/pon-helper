...
class SwitcherCoreActionStorage extends AbstractStorage
{
    ...
    public function add($object)
    {
        ...
        $object->hash = sha1("{$object->getDevice()->getIp()}{$object->getModule()}{$argvJson}");
    ...
    function getLastSuccess(Device $device, $method, $arguments = [])
    {
        ...
        $hash = sha1("{$device->getIp()}{$method}{$argvJson}");
        $psth = $this->pdo->prepare("SELECT id FROM switcher_core_actions WHERE `status` = 'SUCCESS' and `hash` = ? ORDER BY id desc LIMIT 1");
        $psth->execute([$hash]);
        if ($psth->rowCount() === 0) {
